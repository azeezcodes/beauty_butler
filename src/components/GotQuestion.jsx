    ...
    return (
        <div className='flex flex-col lg:flex-row pt-[32px] lg:pt-[45px] md:pt-[36px] bg-[#F6F5FD] max-h-fit gap-3 px-[16px] lg:px-[80px] py-[60px] lg:py-[80px]'>
            <div className='flex-1'>
                <p className='font-apfel font-[400] text-base lg:text-2xl text-[#8877D8]'>Got Questions?</p>
                ...
                {content.map((item) => (
                    <div key={item.id} className='border bg-white rounded-2xl p-4 w-full border-[#8877D8] mb-4'>
                        <div className='flex justify-between items-center'>
                            <p className='font-apfelGrotezkFettMittel  text-[#101828] text-base lg:text-2xl'>{item.title}</p>
                            <div
                            ...
                            >
                                {openItemId !== item.id ? (<img src='/add.svg' alt='plus' />) : (<img src='/minus.svg' alt='minus' />)}
                            </div>
                        </div>
                        {openItemId === item.id && (
                            <p className='text-[#667085] font-euclidRegular lg:text-base text-sm mt-2 leading-4'>
                                {item.text}
                            </p>
                        )}
                    </div>
